...
const { Code, CodeWords } = require('./model');
...
const uploadToMongo = async (docs, docsWithWords, terminology) => {
  await Promise.all([
    insertToMongo(Code, docs, terminology),
    insertToMongo(CodeWords, docsWithWords, terminology),
  ]);
};

const splitDefinitionsIntoWords = (terminology) => {
  Object.keys(mem[terminology]).forEach((v) => {
    const nchar = {};
    const nword = {};
    mem[terminology][v].t.forEach((vv) => {
      if (vv) {
        ...
        pino.info(v, mem[terminology][v]);
      }
    });
    mem[terminology][v].c = Object.keys(nchar);
    mem[terminology][v].words = Object.keys(nword);
  });
};
...
const createDocsForCodeCollection = terminology => Object.keys(mem[terminology]).map((v) => {
  ...
  return {
    _id: { d: terminology, c: v },
    t: mem[terminology][v].t.join('|'),
    a: ancestors,
    p: mem[terminology][v].p,
    c: mem[terminology][v].c,
  };
});

const createDocsForCodeWordsCollection = terminology => Object.keys(mem[terminology]).map(v => ({ _id: { d: terminology, c: v }, t: mem[terminology][v].t.join('|'), words: mem[terminology][v].words }));
...
const processInMemoryTerminologies = () => {
  terminologies.forEach(async (terminology) => {
    if (!mem[terminology]) return;
    splitDefinitionsIntoWords(terminology);

    const docs = createDocsForCodeCollection(terminology);
    const docWithWords = createDocsForCodeWordsCollection(terminology);

    pino.info(`Writing cached doc files for ${terminology}..`);

    await writeDocFiles(docs, path.join(config.CACHED_DIR, terminology, config.CACHED_FILE));
    pino.info(`1 doc file written for ${terminology}..`);
    await writeDocFiles(docWithWords, path.join(config.CACHED_DIR, terminology, `words_${config.CACHED_FILE}`));
    pino.info(`Both doc files written for ${terminology}..`);
...
const processTerminology = function processTerminology(terminology) {
  pino.info(`Processing ${terminology}`);
  try {
    const directory = path.join('terminologies', terminology, 'data-processed');
    fs.readdirSync(directory).forEach((file) => {
      if (file.indexOf('swp') < 0) processDictionaryFile(terminology, directory, file);
    });
  } catch (e) {
...
mongoose.connect(config.MONGO_URL, {
  ...
  if (config.OVERWRITE_FILE) {
    ...
    await Promise.all(terminologies.map(async (terminology) => {
      const [docs, docWithWords] = await loadCachedFiles(terminology);
