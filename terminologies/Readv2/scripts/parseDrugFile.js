...
const run = function run(callback) {
  ...
  const input = fs.createReadStream(path.join('terminologies', 'Readv2', 'data-input', 'unidrug.key'));
  const output = fs.createWriteStream(path.join('terminologies', 'Readv2', 'data-processed', 'unidrug.key.js.dict.txt'));
  output.on('finish', () => {
    console.timeEnd('Drug file elapsed');
    if (callback) return callback();
  });

  console.time('Drug file elapsed');
